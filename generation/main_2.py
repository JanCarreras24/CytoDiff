...
class GenerateImage:
    ...
    def name_template_method(self, classname):
        # Obtener el prompt único para la clase
        specific_prompt = PROMPTS_BY_CLASS.get(classname, f"A generic image of {classname}")
        
        # Repetir el prompt para generar el número requerido de imágenes
        prompts = [specific_prompt] * self.n_img_per_class
        return prompts
...
def main(
    ...
    assert mode == "datadream" and n_shot >= 1, "`n_shot` should be integer when `mode` is datadream."
    if mode == "zeroshot":
        n_shot = 0
    ...
    print('Image generation started using ', device);
